...
# to ask yet.
UNIT_FALLBACK="PRO-UI"
# The management menu (`PRO-UI`). Installed from INSIDE the binary we just placed
...
# in PRO-UI.sh, which is sourced further below once the menu script is installed.
# It used to be defined here and copied into the menu, which is exactly how two
# acme.sh flows drift apart. Sourcing (rather than running `PRO-UI ssl`) keeps it
...
#
# Deliberately NOT named panel_unit: PRO-UI.sh, sourced below for
...
# Also reap a panel launched OUTSIDE systemd (a bare ./PRO-UI): the stop above only
...
if command -v pkill >/dev/null 2>&1; then
    pkill -x PRO-UI 2>/dev/null || true
...
if [[ "$MODE" == "update" && -f "$DB" ]]; then
    ...
    ts="$(date +%Y%m%d-%H%M%S)"
    backup="$BACKUP_DIR/PRO-UI_${OLD_VER:-unknown}_${ts}.db"
...
# `PRO-UI` always matches the binary that ships it. Must come before the TLS step
...
# the compiled-in /opt/PRO-UI default.
export VPNUI_BIN="$DEST"
if "$DEST" install-menu >/dev/null 2>&1 && [[ -r "$MENU" ]]; then
    ok "management menu -> ${MENU}  (run: ${TEAL}PRO-UI${R})"
    # Bring in obtain_letsencrypt_cert: the single implementation, shared rather
    # than copied. PRO-UI.sh does nothing at top level when sourced (its menu is
    # behind a sourced/executed guard), so this only defines functions.
    # shellcheck source=PRO-UI.sh
    source "$MENU"
else
    warn "could not install ${MENU}, so the 'PRO-UI' menu is unavailable on this host."
...
if [[ "$MODE" == "install" ]]; then
    # Optional migration: import an existing 3x-ui (or PRO-UI) backup database before
...
if [[ -x "$MENU" ]]; then
    act "manage:  ${TEAL}PRO-UI${R}  (update, login, start/stop, Xray, SSL)"
fi
