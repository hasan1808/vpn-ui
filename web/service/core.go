...
func (s *CoreService) runProvisionSteps(emit func(ProvisionStep), cores []string) (rebootModules []string, rebootPkg string, failed bool) {
	...
	// in the binary. A build compiled WITHOUT it (backend/bin/<arch> empty, e.g.
	// a from-source build with SKIP_BACKEND=1) has no way to install them: the
	// host-package fallback exists only for STATUS (commandExists), not for
	// install. Report that as a hard failure instead of silently running the rest
	// of the steps, persisting the cores as "installed", and leaving the status
	// cards contradicting the setup console — which is what a silent skip did.
	requested := daemonsFor(target)
	if !backend.Available() {
		// No bundle at all: every requested daemon must come from a host package.
		// That is fine for status but nothing here installs packages, so any
		// requested daemon absent from the host is permanently uninstallable on
		// this build — a hard failure, not the old silent success that persisted
		// the core and left its status card contradicting the green console.
		var missing []string
		...
		if len(missing) > 0 {
			...
			emit(ProvisionStep{Name: "extract bundled daemons", OK: false,
				Msg: "this build has no embedded VPN daemon bundle and the host lacks: " +
	...
	// Install the fwmark policy route that delivers TPROXY'd VPN packets to
	// the Xray dokodemo socket. Without this rule, nftables marks the packets
	// but the kernel never hands them to the local TPROXY listener, so VPN
	// clients connect successfully but get zero data throughput.
	EnsureFwmarkRule()
	emit(ProvisionStep{Name: "ip rule fwmark 1 → table 100", OK: true, Msg: "installed"})

	// Persist fwmark policy route across reboots via rc.local.
	// The ip rule is NOT persisted by sysctl, so it vanishes on reboot
	// without an explicit hook.
	if err := os.WriteFile("/etc/vpn-ui/fwmark-rule.sh", []byte("#!/bin/sh\nip rule add fwmark 1 lookup 100 2>/dev/null\nip route replace local 0.0.0.0/0 dev lo table 100 2>/dev/null\n"), 0755); err == nil {
		rcLocal := "/etc/rc.local"
		entry := "/etc/vpn-ui/fwmark-rule.sh\n"
		if data, readErr := os.ReadFile(rcLocal); readErr != nil || !strings.Contains(string(data), "fwmark-rule.sh") {
			f, fErr := os.OpenFile(rcLocal, os.O_APPEND|os.O_WRONLY|os.O_CREATE, 0755)
			if fErr == nil {
				defer f.Close()
				if len(data) == 0 || strings.TrimSpace(string(data)) != "exit 0" {
		...
		emit(ProvisionStep{Name: "persist fwmark rule across reboots", OK: true, Msg: rcLocal})
	} else {
		emit(ProvisionStep{Name: "persist fwmark rule across reboots", OK: false, Msg: err.Error()})
	}
...
func (s *CoreService) StartProvision(cores []string) bool {
	...
	go func() {
		...
		if failed {
			...
			// fixing it (rebuilding with the daemon bundle or installing host
			// packages) and the recording happens then.
			provisionRun.mu.Lock()
