...
// Asserted on the POINTER, which is what init registers below: an optional interface is
// found by a type assertion on the stored driver, so a method set that only a value
// satisfies would leave the carrier gate silently unasked and PPTP quietly accepted onto a
// carrier that cannot move it.
var _ VpnOutCarriable = (*pptpOutDriver)(nil)
...
// CarriableOverProxy refuses, always: PPTP is the one kind that can never ride a carrier
// which is an Xray outbound.
//
// The protocol is two channels and only one of them is dispatchable. The control channel is
// TCP 1723 and would ride happily; the DATA channel is PPP inside GRE, IP protocol 47, and
// the tun the framework puts in front of an Xray outbound dispatches TCP and UDP only. A raw
// protocol there is swallowed rather than refused - measured, counted on the device's RX,
// none leave, nothing logged - so an accepted PPTP tunnel would authenticate, bring up a ppp
// device, hold a route, and carry not one frame. That is the failure this refusal exists to
// prevent: half of a protocol delivered, with every screen calling it up.
//
// Nothing in the form changes the answer, which is why there is no configuration test here
// and why the message names no setting. The bundled client speaks no proxy and has no UDP
// encapsulation of any kind: its whole outer-path vocabulary is `--localbind <addr>` and
// `--rtmark <n>` (checked against the shipped binary's own usage text), and pppd sits above
// it and never sees the wire at all. The alternative is a carrier that is a DEVICE - another
// VPN tunnel, or a freedom outbound pinned to an interface - because steering into a netdev
// is L4-agnostic and takes the GRE whole. That is what the panel does for PPTP today, and it
// keeps working exactly as it did.
func (d *pptpOutDriver) CarriableOverProxy(cfg VpnOutboundConfig) (bool, string) {
	return false, "PPTP is a TCP 1723 control channel plus its data in GRE, IP protocol 47, and an Xray " +
		"outbound carries a tunnel through a tun that dispatches TCP and UDP only: the control channel " +
		"would connect and every PPP frame would vanish into the carrier unlogged, which is why this is " +
		"refused rather than half-delivered. No setting changes that, because the bundled pptp client " +
		"speaks no proxy and has no UDP encapsulation: carry this tunnel with a VPN tunnel or a freedom " +
		"outbound pinned to an interface, which steer into a device and take the GRE whole"
}
