...
// EnsureFwmarkRule installs the kernel policy-routing rule that delivers TPROXY'd
// VPN client packets to Xray. Every VPN protocol's TPROXY chain marks packets
// with fwmark 1; this rule routes marked packets to table 100, which contains a
// local-delivery route that hands them to the dokodemo-door socket.
//
// Without it, nftables successfully marks and redirects the packets, but the
// kernel never delivers them to the local TPROXY listener — clients connect,
// but zero data flows.
//
// Idempotent: uses ip rule replace semantics (delete-then-add if duplicates
// exist) so repeated calls are safe.
func EnsureFwmarkRule() {
	// Remove any duplicate rules first (the old idempotency check was fragile)
	exec.Command("ip", "rule", "del", "fwmark", "1", "lookup", "100").Run()

	// Install exactly one rule
	if err := exec.Command("ip", "rule", "add", "fwmark", "1", "lookup", "100").Run(); err != nil {
		logger.Warning("nft: could not install fwmark policy route:", err)
	}

	// Ensure table 100 has the local-delivery route
	_ = exec.Command("ip", "route", "replace", "local", "0.0.0.0/0", "dev", "lo", "table", "100").Run()
}
...
func (s *NftService) ApplyNftRules() error {
	...
	// Ensure the fwmark policy route is in place. This is the critical missing
	// link: without it, nftables marks packets but the kernel never delivers
	// them to Xray's TPROXY socket, so VPN clients connect but get no data.
	EnsureFwmarkRule()
